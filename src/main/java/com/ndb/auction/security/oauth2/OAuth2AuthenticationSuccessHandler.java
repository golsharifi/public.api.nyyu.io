...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response,
        ...
        // **CRITICAL FIX: Create JWT token here**
        String token = null;
        if (type.equals("success")) {
            try {
                // Use the UserDetailsImpl instead of Authentication
                token = tokenProvider.createToken(userPrincipal);
                log.info("🎫 Created JWT token for user: {}", user.getEmail());
            } catch (Exception e) {
        ...
        // Add token to the URL if created successfully
        String finalUrl;
        if (token != null) {
