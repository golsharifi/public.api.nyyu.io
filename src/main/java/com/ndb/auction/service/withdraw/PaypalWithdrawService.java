...
public class PaypalWithdrawService extends BaseService {
    ...
    public BaseWithdraw createNewWithdrawRequest(BaseWithdraw baseWithdraw) {
        PaypalWithdraw withdrawRequest = (PaypalWithdraw) baseWithdraw;
        ...
        try {
            ...
            // Enforce verification requirements for PayPal withdrawals
            // Enforce verification requirements for PayPal withdrawals
            ...
            BaseWithdraw result = paypalWithdrawDao.insert(withdrawRequest);

            log.info("PayPal withdrawal request created successfully for user {} amount {}",
                    withdrawRequest.getUserId(), withdrawRequest.getWithdrawAmount());

            return result;

        } catch (VerificationRequiredException e) {
            log.error("Verification required for PayPal withdrawal: user={}, amount={}, error={}",
                    withdrawRequest.getUserId(), withdrawRequest.getWithdrawAmount(), e.getMessage());
            throw e;
        } catch (Exception e) {
            log.error("Error creating PayPal withdrawal for user {}: {}",
