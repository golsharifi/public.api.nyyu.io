...
public class ShuftiDao extends BaseOracleDao {
	...
	public int update(ShuftiReference m) {
		String sql = "UPDATE TBL_SHUFTI_REF SET REFERENCE = ?, VERIFY_TYPE = ?, DOC_STATUS = ?, ADDR_STATUS = ?, CON_STATUS = ?, SEL_STATUS = ?, PENDING = ? WHERE USER_ID = ?";
		return jdbcTemplate.update(sql,
				m.getReference(),
				m.getVerificationType(),
				m.getDocStatus(),
				m.getAddrStatus(),
				m.getConStatus(),
				m.getSelfieStatus(),
	...
	public int insert(ShuftiReference m) {
		String sql = "INSERT INTO TBL_SHUFTI_REF(USER_ID, REFERENCE, VERIFY_TYPE, DOC_STATUS, ADDR_STATUS, CON_STATUS, PENDING)"
				+ "VALUES(?,?,?,?,?,?,?)";
		return jdbcTemplate.update(sql, m.getUserId(), m.getReference(), m.getVerificationType(), m.getDocStatus(),
				m.getAddrStatus(), m.getConStatus(), m.getPending());
	}
	...
	public int updateSelfieStatus(int userId, Boolean status) {
		String sql = "UPDATE TBL_SHUFTI_REF SET SEL_STATUS = ? WHERE USER_ID = ?";
		return jdbcTemplate.update(sql, status, userId);
	}
